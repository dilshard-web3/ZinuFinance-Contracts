...
import {Ownable} from "./Lib8/Ownable.sol";
import {IERC20} from "./Interface8/IERC20.sol";
import {SafeERC20} from "./Lib8/SafeERC20.sol";
import {ReentrancyGuard} from "./Lib8/ReentrancyGuard.sol";

import {IZombieInuPair} from "./Interface/IZombieInuPair.sol";
import {IZombieInuRouter02} from "./Interface/IZombieInuRouter02.sol";
import {IWETH} from "./Interface/IWETH.sol";
import {Babylonian} from "./Lib8/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 */
contract ZombieInuZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // ZombieInuRouter interface
    IZombieInuRouter02 public zombieinuRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from ZinuSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address ZombieInuRouter
    address private zombieinuRouterAddress;
     ...
     * @param _zombieinuRouter: address of the ZombieInuRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _zombieinuRouter,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        zombieinuRouterAddress = _zombieinuRouter;
        zombieinuRouter = IZombieInuRouter02(_zombieinuRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap BNB in a WBNB pool (e.g. WBNB/token)
     * @param _lpToken: LP token address (e.g. ZINU/BNB)
     * @param _tokenAmountOutMin: minimum token amount (e.g. ZINU) to receive in the intermediary swap (e.g. BNB --> ZINU)
     */
     ...
     * @param _lpToken: LP token address (e.g. ZINU/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ZINU) in the intermediary swap (e.g. BUSD --> ZINU)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. ZINU/WBNB)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in BNB/WBNB) in the intermediary swap (e.g. ZINU --> BNB)
     */
    function zapOutBNB(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. ZINU/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. ZINU or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ZINU) in the intermediary swap (e.g. BUSD --> ZINU)
     */
    ...
    {
        address token0 = IZombieInuPair(_lpToken).token0();
        address token1 = IZombieInuPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IZombieInuPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = zombieinuRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = zombieinuRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IZombieInuPair(_lpToken).token0() || _token0ToZap == IZombieInuPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IZombieInuPair(_lpToken).token0() || _token1ToZap == IZombieInuPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IZombieInuPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IZombieInuPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IZombieInuPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = zombieinuRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = zombieinuRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IZombieInuPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "ZombieInuRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "ZombieInuRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        zombieinuRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), zombieinuRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(zombieinuRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = zombieinuRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = zombieinuRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = zombieinuRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = zombieinuRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = zombieinuRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
