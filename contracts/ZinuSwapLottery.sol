...
import "./Interface8/IZinuSwapLottery.sol";
...
/** @title ZinuSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract ZinuSwapLottery is ReentrancyGuard, IZinuSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInZinu = 50 ether;
    uint256 public minPriceTicketInZinu = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInZinu;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] zinuPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInZinu;
        uint32 finalNumber;
    }
     ...
     * @param _zinuTokenAddress: address of the ZINU token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _zinuTokenAddress, address _randomGeneratorAddress) {
        zinuToken = IERC20(_zinuTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of ZINU to this contract
        uint256 amountZinuToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInZinu,
            _ticketNumbers.length
        );

        // Transfer zinu tokens to this contract
        zinuToken.safeTransferFrom(address(msg.sender), address(this), amountZinuToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInZinu += amountZinuToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInZinuToTransfer
        uint256 rewardInZinuToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInZinuToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        zinuToken.safeTransfer(msg.sender, rewardInZinuToTransfer);

        emit TicketsClaim(msg.sender, rewardInZinuToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in ZINU per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInZinu) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in ZINU for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].zinuPerBracket[j] =
                ...
                // A. No ZINU to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].zinuPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInZinu - amountToShareToWinners);

        // Transfer ZINU to treasury address
        zinuToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in ZINU token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        zinuToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInZinu += _amount;
     ...
     * @param _priceTicketInZinu: price of a ticket in ZINU
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInZinu >= minPriceTicketInZinu) && (_priceTicketInZinu <= maxPriceTicketInZinu),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInZinu: _priceTicketInZinu,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            zinuPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInZinu: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(zinuToken), "Cannot be ZINU token");
    ...
    /**
     * @notice Set ZINU price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInZinu: minimum price of a ticket in ZINU
     * @param _maxPriceTicketInZinu: maximum price of a ticket in ZINU
     */
    function setMinAndMaxTicketPriceInZinu(uint256 _minPriceTicketInZinu, uint256 _maxPriceTicketInZinu)
        external
        onlyOwner
    {
        require(_minPriceTicketInZinu <= _maxPriceTicketInZinu, "minPrice must be < maxPrice");

        minPriceTicketInZinu = _minPriceTicketInZinu;
        maxPriceTicketInZinu = _maxPriceTicketInZinu;
    }
     ...
     * @param _priceTicket price of a ticket (in ZINU)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].zinuPerBracket[_bracket];
        } else {
            return 0;
        }
    }
